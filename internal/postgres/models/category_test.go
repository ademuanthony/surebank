...
func testCategoryToManyProductCategories(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CategorySlice{&a}
	if err = a.L.LoadProductCategories(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCategoryToManyAddOpProductCategories(t *testing.T) {
	var err error
...
var (
	categoryDBTypes = map[string]string{`ID`: `character`, `Name`: `character varying`}
	_               = bytes.MinRead
)
