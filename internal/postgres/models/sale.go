...
var SaleTableColumns = struct {
	...
	Amount:        "sale.amount",
	AmountTender:  "sale.amount_tender",
	...
	UpdatedByID:   "sale.updated_by_id",
	ArchivedByID:  "sale.archived_by_id",
}
...
func (saleL) LoadPayments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSale interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`payment`),
		qm.WhereIn(`payment.sale_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (saleL) LoadSaleItems(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSale interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`sale_item`),
		qm.WhereIn(`sale_item.sale_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
