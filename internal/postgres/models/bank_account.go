...
type (
	...
	// This should generally be used opposed to []BankAccount.
...
func (bankAccountL) LoadBankDeposits(ctx context.Context, e boil.ContextExecutor, singular bool, maybeBankAccount interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`bank_deposit`), qm.WhereIn(`bank_deposit.bank_account_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
