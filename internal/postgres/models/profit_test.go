...
func testProfitsExists(t *testing.T) {
	...
	e, err := ProfitExists(ctx, tx, o.ID)
	if err != nil {
...
func testProfitsFind(t *testing.T) {
	...
	profitFound, err := FindProfit(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
var (
	profitDBTypes = map[string]string{`ID`: `character`, `Amount`: `double precision`, `Narration`: `character varying`, `CreatedAt`: `bigint`, `UpdatedAt`: `bigint`, `ArchivedAt`: `bigint`}
	_             = bytes.MinRead
)
